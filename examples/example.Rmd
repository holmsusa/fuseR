...
```{r}
k0 <- read.table('../../methylation/manuscript/simulated_dataset/data0/k0_data0.csv', header = T, row.names = 1)
k1 <- read.table('../../methylation/manuscript/simulated_dataset/data0/k1_data0.csv', header = T, row.names = 1)
#k0 <- read.table('k0.tsv', header = T, row.names = 1)
#k1 <- read.table('k1.tsv', header = T, row.names = 1)
...
betas_df <- betas[which(segments %in% c(1:20)), ]
location_df <- location[1:20, ]
...
ggplot() +
  ...
  labs(x = "Genomic Position", y = "Beta", title = "Beta Segments") +
  theme_minimal()
